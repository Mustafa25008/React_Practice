...
interface EditModalProps {
    ...
    setmsg: (msg: { message: string; type: string }) => void;
    prod: Products_items;
    setprod: (prod: Products_items) => void;
}
...
function EditModal ({modalShow, setModal, setmsg, prod, setprod}: EditModalProps){
    ...
    const handleClose = () => {
        setModal(false);
        setprod({id:null, title:"", price:null, description:""});
    };

    async function saveProduct(data: Products_items) {
        try {
            ...
            <Modal show={modalShow} onHide={handleClose}>
                <form onSubmit={handleSubmit(saveProduct)}>
                    ...
                    <Modal.Body className="d-flex gap-0 flex-column">
                        ...
                        {errors.title && <span style={{ color: 'red' }}>{String(errors.title.message)}</span>}
                        <div className="d-flex gap-2 m-2">
